...
const DialogSubscribers = () => {
  const [open, setOpen] = useState(false);
  const [filter, setFilter] = useState("");
  const { data, error, isLoading } = useSWR("getAllProfiles", async () => {
  ...
  return (
    <div>
      <Button full className="!bg-white py-6" onClick={() => setOpen(true)}>
      ...
      <MyDialog open={open} setOpen={setOpen} title="Subscribers">
        <div className="py-4 px-6 min-w-[28rem] space-y-4">
          <Input
            ...
            type="text"
            placeholder="Search"
            className="focus-within:ring-blue focus-within:border-blue"
            adornment={
              <svg
                width="18"
                height="19"
            ...
            {isLoading ? (
              ...
              <ul className="space-y-3 divide-y divide-slate-200">
                {data
                  ...
                  .map((profile, key) => (
                    <li key={key} className="flex space-x-4 items-center pt-3">
                      <img
...
export default DialogSubscribers;
